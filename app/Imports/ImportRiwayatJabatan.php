...
$tez = collect();
       ...
       if (count($tanggal) < 3) {
        ...
        if(is_int(rtrim($ar['awal_menjabat']))){
          $UNIX_DATE = ($ar['awal_menjabat'] - 25569) * 86400;
          $EXCEL_DATE = 25569 + ($UNIX_DATE / 86400);
          $UNIX_DATE = ($EXCEL_DATE - 25569) * 86400;
          $awal_menjabat =  gmdate("d/m/Y", $UNIX_DATE);
        } else{
          $awal_menjabat = !empty($ar['awal_menjabat']) ? rtrim(\Carbon\Carbon::createFromFormat('d/m/Y', $ar['awal_menjabat'])->format('Y-m-d')) : NULL;
        }

        if(is_int(rtrim($ar['akhir_menjabat']))){
          $UNIX_DATE = ($ar['akhir_menjabat'] - 25569) * 86400;
          $EXCEL_DATE = 25569 + ($UNIX_DATE / 86400);
          $UNIX_DATE = ($EXCEL_DATE - 25569) * 86400;
          $akhir_menjabat =  gmdate("d/m/Y", $UNIX_DATE);
        } else{
          $akhir_menjabat = !empty($ar['akhir_menjabat']) ? rtrim(\Carbon\Carbon::createFromFormat('d/m/Y', $ar['akhir_menjabat'])->format('Y-m-d')) : NULL;
        }
